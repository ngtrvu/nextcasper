...
const calcHash = async (input) => await sha1(input)
...
export const imageDimensions = async (url, noCache) => {
  if (!url) return null
...
export const imageDimensionsFromFile = async (file, noCache) => {
  if (!file) return null
...
export const normalizedImageUrl = async (url) => {
  var _a
  const localhostRegExp = /^http:\/\/\w+(\.\w+)*(:[0-9]+)?\/?(\/.*)*\/(.*)$/
  const filename =
    (_a = url.match(localhostRegExp)) === null || _a === void 0
      ? void 0
      : _a.reverse()[0]
